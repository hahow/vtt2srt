...
class LineGroup {
  ...
  _parseTimestamp (time) {
    let [startTime, endTime] = time.split('-->').map(token => token.trim())

    let [sHour, sMinute, sSecondWithMS] = this._fillHourIfNotExists(startTime)
    let [eHour, eMinute, eSecondWithMS] = this._fillHourIfNotExists(endTime)

    return `${sHour}:${sMinute}:${sSecondWithMS} --> ${eHour}:${eMinute}:${eSecondWithMS}`
  }

  _fillHourIfNotExists (time) {
    let [hour, minute, secondWithMS] = time.match(TIME_REGEXP).slice(2)
